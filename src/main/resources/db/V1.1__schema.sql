CREATE TABLE IF NOT EXISTS `sys_ad_cd` (
...
CREATE TABLE IF NOT EXISTS `sys_user` (
  ...
  `IS_ENABLED` int(11) NOT NULL COMMENT '是否启用 0禁用1启用',
  `UPDATE_TIME` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '用户更新时间',
  `CREATE_TIME` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '用户创建时间',
...
CREATE TABLE IF NOT EXISTS `sys_user_adcd` (
...
CREATE TABLE IF NOT EXISTS `sys_log` (
  ...
  `etime` datetime DEFAULT NULL COMMENT '访问结束时间',
